namespace MyDevTools
{
    ...
    public partial class MyDevToolsForm
    {
        ...
        bool IsBuildFrontSolution { get; set; }
        ...
        void LoadSolutionsFiles()
        {
            ...
            if (!string.IsNullOrWhiteSpace(MyDevToolsPlugin.DefaultSolutionFileName.ValueOrDefault(MyDevToolsPlugin.PluginSettings)) && CblSolutions.Items.Count > 1)
            {
                foreach (var item in CblSolutions.Items)
                {
                    if (item.ToString().EndsWith(MyDevToolsPlugin.DefaultSolutionFileName.ValueOrDefault(MyDevToolsPlugin.PluginSettings), StringComparison.InvariantCultureIgnoreCase))
                    {
        ...
        void ResetCheckboxBackColor()
        {
            ...
            CbxIsBuildFrontSolution.BackColor = Color.Transparent;
        ...
        void RunProcess()
        {
            ...
            Invoke((MethodInvoker)(() =>
            {
                ...
                if (CbxIsBuildFrontSolution.Checked || CbxIsBuildSolution.Checked || CbxIsGitClean.Checked)
                {
                    TbxLogInfo.AppendText(string.Format("\r\nTarget solution: {0}\r\n", TargetSolutionName));
                }
            }));
            ...
            if (IsGitClean)
            {
                ...
                if (!string.IsNullOrWhiteSpace(MyDevToolsPlugin.ExcludePatternGitClean.ValueOrDefault(MyDevToolsPlugin.PluginSettings)))
                {
                    excludeCommand = $"{MyDevToolsPlugin.ExcludePatternGitClean.ValueOrDefault(MyDevToolsPlugin.PluginSettings)}";
                }
                Invoke((MethodInvoker)(() =>
                {
            ...
            if (IsPreBuildSolution)
            {
                ...
                bool result = GenericHelper.RunCommandLine(PreBuildFiles.ToList(), string.Empty, ref errorMessages);
                if (!result)
                {
                    Invoke((MethodInvoker)(() =>
                    {
            ...
            if (IsNugetRestore)
            {
                ...
                string errorMessages = string.Empty;
                if (GenericHelper.RunCommandLine(new List<string> { $"nuget restore {solutionFullPath}" }, string.Empty, ref errorMessages))
                {
                    Invoke((MethodInvoker)(() =>
                    {
            ...
            if (IsRestoreDatabase && Databases.Any())
            {
                ...
                foreach (var database in Databases)
                {
                    ...
                    if (DatabaseHelper.RestoreDatabase(database.DatabaseName, database.BackupFilePath,
                        MyDevToolsPlugin.DatabaseServerName.ValueOrDefault(MyDevToolsPlugin.PluginSettings),
                        MyDevToolsPlugin.DatabaseRelocateFile.ValueOrDefault(MyDevToolsPlugin.PluginSettings),
                        MyDevToolsPlugin.DatabaseRelocateFile.ValueOrDefault(MyDevToolsPlugin.PluginSettings), ref errorMessages))
                    {
                        isRestore = true;
                        Invoke((MethodInvoker)(() =>
                        {
            ...
            if (IsBuildSolution)
            {
                Invoke((MethodInvoker)(() =>
                {
                    ...
                    TbxLogInfo.AppendText($"\r\nBuilding solution 'Back': {TargetSolutionName}...");
                }));
                ...
                if (!GenericHelper.InvokeMsBuild(solutionFullPath, Generic.GenrateSolutionArguments.Build, ref errorResult))
                {
                    Invoke((MethodInvoker)(() =>
                    {
                        ...
                        TbxLogInfo.AppendText($"\r\nError when building solution 'Back': {solutionFullPath}.");
                        TbxLogInfo.AppendText($"\r\n{errorResult}");
                    }));
                }
                else
            ...
            if (IsBuildFrontSolution)
            {
                Invoke((MethodInvoker)(() =>
                {
                    CbxIsBuildFrontSolution.BackColor = Generic.ColorProcessTaskInProgress;
                    TbxLogInfo.AppendText($"\r\nBuilding solution 'Front': {TargetSolutionName}...");
                }));
                string errorResult = string.Empty;
                if (!GenericHelper.InvokeYarn(solutionFullPath, ref errorResult))
                {
                    Invoke((MethodInvoker)(() =>
                    {
                        CbxIsBuildFrontSolution.BackColor = Generic.ColorProcessTaskFailed;
                        TbxLogInfo.AppendText($"\r\nError when building solution 'Front': {solutionFullPath}.");
                        TbxLogInfo.AppendText($"\r\n{errorResult}");
                    }));
                }
                else
                {
                    Invoke((MethodInvoker)(() =>
                    {
                        CbxIsBuildFrontSolution.BackColor = Generic.ColorProcessTaskSuccess;
                    }));
                }
            }
            if (IsPostBuildSolution)
            {
                ...
                string errorMessages = string.Empty;
                bool result = GenericHelper.RunCommandLine(PostBuildFiles.ToList(), string.Empty, ref errorMessages);
                if (!result)
                {
                    Invoke((MethodInvoker)(() =>
                    {
        ...
        private void BtnRunProcessClick(object sender, EventArgs e)
        {
            ...
            IsBuildFrontSolution = CbxIsBuildFrontSolution.Checked;
        ...
        void CbxIsCreateNewBranchCheckedChanged(object sender, EventArgs e)
        {
            if (CbxIsCreateNewBranch.Checked)
            {
                ...
                if (!string.IsNullOrWhiteSpace(MyDevToolsPlugin.NewBranchPrefix.ValueOrDefault(MyDevToolsPlugin.PluginSettings)))
                {
                    TxbNewBranchName.Text = MyDevToolsPlugin.NewBranchPrefix.ValueOrDefault(MyDevToolsPlugin.PluginSettings);
                }
            }
            else
            {
