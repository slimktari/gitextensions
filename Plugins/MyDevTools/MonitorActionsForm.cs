namespace MyDevTools
{
    ...
    public partial class MonitorActionsForm : Form
    {
        ...
        {
            MyDevToolsPlugin.BranchesToMonitor[_settings] = string.Join(";", MyDevToolsPlugin.BranchesToMonitor.ValueOrDefault(MyDevToolsPlugin.PluginSettings).Split(';').Where(x => !string.IsNullOrWhiteSpace(x)).Where(x => x != _currentItem.LocalName).ToList());
