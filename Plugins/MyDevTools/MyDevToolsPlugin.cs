namespace MyDevTools
{
    ...
    public class MyDevToolsPlugin : GitPluginBase, IGitPluginForRepository
    {
        ...
        public static BoolSetting IsDefaultBuildFrontSolution = new BoolSetting("Is default build front solution", false);
        ...
        {
            SetNameAndDescription(Generic.PluginName);
            //Translate();
        }
        ...
        {
            CancelBackgroundOperation();
            int fetchInterval = CheckInterval.ValueOrDefault(PluginSettings);
                    ...
                    Observable.Timer(TimeSpan.FromSeconds(fetchInterval))
                        .SkipWhile(
                            i =>
                                ...
                                CheckInterval.ValueOrDefault(PluginSettings) == Generic.DisableValueCheckMonitoInterval)
                        .Repeat()
        ...
        void MonitorTask()
        {
            if (!string.IsNullOrWhiteSpace(BranchesToMonitor.ValueOrDefault(PluginSettings)))
            {
                var tab = BranchesToMonitor.ValueOrDefault(PluginSettings).Split(';').Where(x => !string.IsNullOrWhiteSpace(x)).ToList();
                if (tab.Any())
                {
